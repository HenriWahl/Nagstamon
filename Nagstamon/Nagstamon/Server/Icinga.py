...
class IcingaServer(GenericServer):    
    ...
    def _get_status_JSON(self):
        ...
        # since version 1.7 CGI parameters slightly changed, most important is the fact that hosts without hostgroups
        # would not be displayed if the hostgroup parameter would be used
        if self.version < "1.7":
            ...
            nagcgiurl_services = "%s/status.cgi?style=servicedetail&servicestatustypes=%s&serviceprops=%s&jsonoutput" % (self.monitor_cgi_url,\
                                                                                                              str(servicestatustypes),\
                                                                                                              str(hostserviceprops))
            # hosts (up or down or unreachable)
            nagcgiurl_hosts = "%s/status.cgi?style=hostdetail&hoststatustypes=%s&hostprops=%s&jsonoutput" % (self.monitor_cgi_url,\
                                                                                                                           str(hoststatustypes),\
                                                                                                                           str(hostserviceprops))
    ...
    def _get_status_HTML(self):
        ...
        if self.version < "1.7":
            # services (unknown, warning or critical?)
            nagcgiurl_services = "%s/status.cgi?host=all&servicestatustypes=%s&serviceprops=%s" % (self.monitor_cgi_url,\
                                                                                                              str(servicestatustypes),\
                                                                                                              str(hostserviceprops))
            # hosts (up or down or unreachable)
            nagcgiurl_hosts = "%s/status.cgi?hostgroup=all&style=hostdetail&hoststatustypes=%s&hostprops=%s" % (self.monitor_cgi_url,\
                                                                                                                       str(hoststatustypes),\
                                                                                                                       str(hostserviceprops))
        else:
            # services (unknown, warning or critical?)
            nagcgiurl_services = "%s/status.cgi?style=servicedetail&servicestatustypes=%s&serviceprops=%s" % (self.monitor_cgi_url,\
                                                                                                   str(servicestatustypes),\
                                                                                                   str(hostserviceprops))
            # hosts (up or down or unreachable)
            nagcgiurl_hosts = "%s/status.cgi?style=hostdetail&hoststatustypes=%s&hostprops=%s" % (self.monitor_cgi_url,\
                                                                                                                str(hoststatustypes),\
                                                                                                                str(hostserviceprops))
        # hosts - mostly the down ones
